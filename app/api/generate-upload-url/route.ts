...
export async function POST(req: Request) {
    try {
        const { filename, contentType, language, level } = await req.json();
        ...
        const objectKey = `materials/${Date.now()}-${sanitizedFilename}`;
        ...
        return NextResponse.json({ success: false, error: err.message || "Failed to generate signed URL" }, { status: 500 });
    }
}
